...
import { Err, ErrFromText, isErr, isOk, Ok, OkState, Result, unwrap } from './index.ts';
...
test('divide() returns correct ErrorState', () => {
  ...
  deepEqual(failedResult, { ok: undefined, error: new DivisionError("Cannot Divide By Zero") });
...
test('Prettify<OkState<number>> is OkState<number>', () => {
  const ResultFunction = (a: OkState<number>): OkState<number> => {
    return a;
  };
